...
__STATIC_INLINE void pin_out_init(GPIO_TypeDef* GPIOx, uint8_t pin_bit)
{
	...
	GPIOx->MODER |= ( ((uint32_t)(0x01) & 0x03) << (pin_bit << 1) );
	GPIOx->OTYPER &= ~(0x00000001 << pin_bit);
	GPIOx->OSPEEDR |= ( ((uint32_t)(0x03) & 0x03) << (pin_bit << 1) );
...
__STATIC_INLINE void pin_out_od_init(GPIO_TypeDef* GPIOx, uint8_t pin_bit)
{
    GPIOx->MODER &= ~(0x00000003 << (pin_bit << 1));
	GPIOx->MODER |= ( ((uint32_t)(0x01) & 0x03) << (pin_bit << 1) );
	GPIOx->OTYPER |= ( ((uint32_t)(0x01) & 0x01) << pin_bit );
	GPIOx->OSPEEDR |= ( ((uint32_t)(0x03) & 0x03) << (pin_bit << 1) );
	GPIOx->PUPDR &= ~(0x00000003 << (pin_bit << 1));
}
...
__STATIC_INLINE void pin_in_init(GPIO_TypeDef* GPIOx, uint8_t pin_bit, uint8_t mode)
{
	...
	GPIOx->PUPDR &= ~(0x00000003 << (pin_bit << 1));
    if(mode == 1)
		GPIOx->OSPEEDR |= ( ((uint32_t)(0x01) & 0x03) << (pin_bit << 1) );//Up
    else if(mode == 2)
		GPIOx->OSPEEDR |= ( ((uint32_t)(0x02) & 0x03) << (pin_bit << 1) );//down
    else
		GPIOx->MODER |= ( ((uint32_t)(0x03) & 0x03) << (pin_bit << 1) );//GPIO_Mode_AIN
}
//**************************************************************************************************
/**
...
__STATIC_INLINE void PORT_JTAG_SETUP(void)
{
    ...
    // nTRST output
    pin_out_od_init(nTRST_PIN_PORT, nTRST_PIN_Bit);
    nTRST_PIN_PORT->BSRR = nTRST_PIN;
...
__STATIC_INLINE void PORT_SWD_SETUP(void)
{
    ...
    pin_in_init(nTRST_PIN_PORT, nTRST_PIN_Bit, 0);
    pin_in_init(JTAG_TDO_PIN_PORT, JTAG_TMS_PIN_Bit, 0);
...
{
    SWCLK_TCK_PIN_PORT->BSRR = ((uint32_t)SWCLK_TCK_PIN << 16);
}
...
{
    SWDIO_OUT_PIN_PORT->BSRR = ((uint32_t)SWDIO_OUT_PIN << 16);
}
...
__STATIC_FORCEINLINE void PIN_TDI_OUT(uint32_t bit)
{
    ...
    if (bit & 1)
        ...
        JTAG_TDI_PIN_PORT->BSRR = ((uint32_t)JTAG_TDI_PIN << 16);
#endif	
}
...
{
#if ( DAP_JTAG != 0 )
    return ((nTRST_PIN_PORT->IDR & nTRST_PIN) ? 1 : 0);
#else
	return 0;
#endif
}
...
__STATIC_FORCEINLINE void PIN_nTRST_OUT(uint32_t bit)
{
    ...
    if (bit & 1)
        ...
        nTRST_PIN_PORT->BSRR = ((uint32_t)nTRST_PIN << 16);
#endif
}
...
__STATIC_FORCEINLINE void     PIN_nRESET_OUT(uint32_t bit)
{
    if (bit & 1)
        ...
        nRESET_PIN_PORT->BSRR = ((uint32_t)nRESET_PIN << 16);
}
...
{
    if (bit & 1)
        CONNECTED_LED_PORT->BSRR = ((uint32_t)CONNECTED_LED_PIN << 16); // LED on
    else
...
__STATIC_INLINE void LED_RUNNING_OUT(uint32_t bit)
{
    if (bit & 1)
		...
		RUNNING_LED_PORT->BSRR = ((uint32_t)RUNNING_LED_PIN << 16); // LED off
}
...
__STATIC_INLINE void DAP_SETUP(void)
{
    ...
    pin_in_init(JTAG_TDI_PIN_PORT, JTAG_TDI_PIN, 0);
    JTAG_TDI_PIN_PORT->BSRR = JTAG_TDI_PIN;

    pin_in_init(JTAG_TDO_PIN_PORT, JTAG_TDO_PIN, 0);

    pin_in_init(JTAG_TCK_PIN_PORT, JTAG_TCK_PIN_Bit, 0);
    JTAG_TCK_PIN_PORT->BSRR = JTAG_TCK_PIN;

    pin_in_init(JTAG_TMS_PIN_PORT, JTAG_TMS_PIN_Bit, 0);
    JTAG_TMS_PIN_PORT->BSRR = JTAG_TMS_PIN;
